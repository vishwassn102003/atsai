...
def calculate_ats_score(resume_text: str, job_desc: str, mode: str,
    ...
    client = _get_model(api_key, model)

    if mode == 'with_jd':
        prompt = f"""You are an expert ATS evaluator. Analyse the resume against the job description.
...
{{
  ...
  "suggestions": [
    ...
    "Suggestion 5"
        ...
        prompt = f"""You are an expert ATS evaluator. Analyse the resume on general ATS best practices.
